...
int JetPhaseSpace::Init(PHCompositeNode *topNode)
{
    ...
    //For PYTHIA trigger
    jetTriggerPythia = new JetTriggerPythia(0.2, 0.0, 7.9, 0.0, 1.0, 1);
                                   ...
                                   TH1F *hPtReco)
{
    ...
    std::vector<particles> constituent_list;
    constituent_list.clear();
    jetAnalyzer->GetAntiKtCommon(good_particle_list, 0.2, 3.0, 7.9, 0.2, 0.7, sim_jet_list, constituent_list, false);
