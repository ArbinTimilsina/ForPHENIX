...
pAuJet::pAuJet(std::string outfilename)
    : SubsysReco("pAuJet"),
      verbo(1),
...
int pAuJet::Init(PHCompositeNode *topNode)
{
    if (verbo)
        {
...
int pAuJet::InitRun(PHCompositeNode *topNode)
{
    if (verbo)
        {
...
int pAuJet::process_event(PHCompositeNode *topNode)
{
    //p+Au runs
...
int pAuJet::ResetEvent(PHCompositeNode *topNode)
{
    tracks_list.clear();
    clusters_list.clear();
    jets_list.clear();
...
int pAuJet::EndRun(const int runNumber)
{
    if (verbo)
        {
...
int pAuJet::End(PHCompositeNode *topNode)
{
    if (verbo)
        {
...
void pAuJet::GetTracks(PHCompositeNode *topNode, std::vector<tracks>& track_list)
{
    ...
    for (unsigned int i = 0; i < phcentral->get_npart(); i++)
        {
            ...
            bool pc3Matching = (fabs(pc3dphi) < 0.00784) && (fabs(pc3dz) < 6.708);
            bool emcMatching = (fabs(emcdphi) < 0.0208) && (fabs(emcdz) < 11.766);
...
void pAuJet::GetClusters(PHCompositeNode *topNode, std::vector<clusters>& cluster_list, float vertex)
{
...
void pAuJet::InitTrees(bool writeTrees)
{
    if(writeTrees)
        {
...
void pAuJet::FillTrees(bool writeTrees)
{
    if(writeTrees)
        {
            //tTracks->Fill();
