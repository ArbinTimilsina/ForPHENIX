...
void makeMap()
{
  ...
  //Set Hot
  cout << "Reading the hot/dead list" << endl;

  ifstream fhot;
  fhot.open("DeadWarnHe3Au.txt");

  if (!fhot)
    {
      cout << "ERROR!!! Hot/dead list file could not be located!" << endl;
    }

  string line;
  while (getline(fhot, line))
    {
      istringstream one_line(line);
      int as, y, z;
      if (one_line >> as >> y >> z)
        {
	    warnmap[as][y][z] |= MASK_HOT;
        }
    }
  ...
  ofstream myfileWarn ("/direct/phenix+u/arbint/Jets/Analysis/code/He3Jet/HotDeadMap/warnmap.txt");
  ...
  for (int ias = 0; ias < NARMSECT; ias++)
    ...
    for (int iarm = 0; iarm < 2; iarm++)
    {
        ...
        canvas[iarm]->SaveAs(Form("/direct/phenix+u/arbint/Jets/Analysis/code/He3Jet/HotDeadMap/Map_Arm%d.png", iarm));
    }
}
