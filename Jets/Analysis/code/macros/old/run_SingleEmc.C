void run_SingleEmc(char* outfile = "EmcStudy.root", int nevents = 10000)
{
  gSystem->Load("libfun4all.so");
  gSystem->Load("librecal.so");
  gSystem->Load("libert.so");
  gSystem->Load("libemcStudy.so");
  ...
  SubsysReco *EmcStudy = new EmcStudy(outfile);
  EmcStudy->Verbosity(1);
  se->registerSubsystem(EmcStudy);
