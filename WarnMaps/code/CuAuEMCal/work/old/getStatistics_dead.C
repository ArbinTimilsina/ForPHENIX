...
void getStatistics_dead()
{
  ...
  const char *ebin[10] = {"0.5", "0.7", "1.0", "1.5", "2.0", "3.0", "5.0", "7.0"};
  ...
  int dead[NARMSECT][NECOREBIN][NSIGMA];
  for (int ias = 0; ias < NARMSECT; ias++)
    {
      for (int iec = 0; iec < NECOREBIN; iec++)
        {
                      ...
                      for (int isg = 0; isg < NSIGMA; isg++)
                        {
                          ...
                          total[ias][iec][isg]++;
                          if (nhitDead < limit_high) dead[ias][iec][isg]++;
                        }
                    }
                }
            }
      ...
      for (int isg = 0; isg < NSIGMA; isg++)
        {
          ...
          hStatistics[ias][isg] = new TH1D(hname, hname, NECOREBIN-1, 0, NECOREBIN-1);
          hStatistics[ias][isg]->SetTitle(Form("%s", armsect[ias]));
          hStatistics[ias][isg]->SetMarkerStyle(2);
          hStatistics[ias][isg]->SetMarkerColor(isg + 1);
          if (isg == 4) hStatistics[ias][isg]->SetMarkerColor(6);
	  hStatistics[ias][isg]->GetYaxis()->SetRangeUser(0, 5);
          hStatistics[ias][isg]->GetYaxis()->SetTitle("% of dead towers");
          hStatistics[ias][isg]->GetXaxis()->SetLabelSize(0.05);
          hStatistics[ias][isg]->GetYaxis()->SetTitleSize(0.035);

          for (int iec = 0; iec < NECOREBIN-1; iec++)
            {
              hStatistics[ias][isg]->GetXaxis()->SetBinLabel(iec + 1, ebin[iec]);

	      hStatistics[ias][isg]->SetBinContent(iec + 1,(float)(dead[ias][iec][isg])*100/total[ias][iec][isg]);
            }
        }
    }
  ...
  cstats->SaveAs("/direct/phenix+hhj/arbint/plots/WarnMaps/statistics_dead.png");
  //  cstats->Close();
}
